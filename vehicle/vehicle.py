...
from common import VehicleReturnAndPrepareStartEvent
from common import VehiclePrepareFinishEvent
...
class Vehicle:
    ...
    def departure_gate_position_event_update(self):
        self.__timestamp = self.__trip.get_depature_timestamp()
    ...
    def arrival_gate_position_event_update(self):
        ...
        self.__timestamp = self.__trip.get_arrival_timestamp()
    ...
    def service_event_update(self):
      if self.__trip.get_service_time()==0:
         ...
         if self.__aircraft.get_flight().get_category()==0:
           self.__aircraft.get_flight().set_scheduled_arrival_timestamp()
         else:
           self.__aircraft.get_flight().set_scheduled_departure_timestamp()
      
      self.__timestamp = self.__trip.get_service_finish_timestamp()
    ...
    def generate_vehicle_return_event(self):
        ...
        self.__current_event = event
   """     
    def generate_return_and_prepare_start_event(self):
        event = VehicleReturnAndPrepareStartEvent(self)
        self.__event_list.append(event)
        self.__current_event = event
    
    def return_and_prepare_start_event_update(self):
        self.__x = 1
        self.__y = 1
        self.__timestamp = self.__trip.get_prepare_start_timestamp()
    
    def generate_prepare_finish_event(self):
        event = VehiclePrepareFinishEvent(self)
        self.__event_list.append(event)
        self.__current_event = event
        
    def prepare_finish_event_update(self):
        #self.__servicestatus = 0
        self.__timestamp = self.__trip.get_prepare_finish_timestamp()
    ...
    def status_turn_to_0_event_update(self):
        self.__timestamp = self.__trip.get_status_turn_to_0_timestamp()
        self.__servicestatus = 0
        self.__trip_list.append(self.__trip)
        self.__trip = None