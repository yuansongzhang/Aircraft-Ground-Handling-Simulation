...
class VehicleLaji:
    ...
    def service_event_update(self):
      if self.__trip.get_service_time()==0:
         ...
         self.__aircraft.set_laji_status(1)
   ...
   """     
    ...
    def service_finish_event_update(self):
        ...
        self.__aircraft.set_laji_status(0) #服务已经完成
        if self.__aircraft.get_flight().get_category()==1: #离港
            self.__aircraft.set_departure_laji_finish_timestamp(self.__timestamp)
        else: #到港
            self.__aircraft.set_arrive_laji_finish_timestamp(self.__timestamp) 
