# -*- coding: utf-8 -*-
from common import VehicleBaiduServiceEvent
...
class VehicleBaidu:
    def __init__(self,name,v,x,y,aircraft,timestamp,servicestatus):  #x,y,aircraft,timestamp,servicestatus需要变
        ...
        self.__vehicle_type = 4 #加油车为3,摆渡车为4
    """  
    ...
    def generate_service_event(self):
        event = VehicleBaiduServiceEvent(self)
        self.__event_list.append(event)
        self.__current_event = event
    ...
    def service_event_update(self):
      if self.__trip.get_service_time()==0:
         ...
         self.__aircraft.set_baidu_status(1)
   ...
   """     
    ...
    def service_finish_event_update(self):
        ...
        self.__aircraft.set_baidu_status(0) #服务已经完成
        if self.__aircraft.get_flight().get_category()==1: #离港
            self.__aircraft.set_departure_baidu_finish_timestamp(self.__timestamp) 
        else: #到港
            self.__aircraft.set_arrive_baidu_finish_timestamp(self.__timestamp) 
